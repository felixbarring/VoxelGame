  ...
  , m_parentWidgetGroup{parent}
{
...

 {
   ...
   auto button3 = make_shared<Button>(2, 325, 270, 150, 30, m_graphicsManager,
       observer, "Graphics");
   auto button4 = make_shared<Button>(3, 325, 230, 150, 30, m_graphicsManager,
       observer, "Audio");
   auto button5 = make_shared<Button>(4, 325, 190, 150, 30, m_graphicsManager,
       observer, "Back");
...

 {
   ...
   auto label = make_shared<Label>(325, 390, 150, 30, " - Game - ",
       m_graphicsManager);
   auto button = make_shared<Button>(0, 325, 230, 150, 30, m_graphicsManager,
       observer, "Back");
...

 {
   ...
   // TODO Refactor?
   shared_ptr<IWidget> label(new Label{325, 390, 150, 30, " - Input - ", m_graphicsManager});

   m_mouseSensitivitySlider = make_shared<Slider>(1, 125 + 100, 310, 150, 30,
       m_graphicsManager, observer, 1);
   m_mouseSensitivityInput = make_shared<TextInput>(666, 125 + 100, 270, 150,
       30, m_graphicsManager, 1);

   auto button1 = make_shared<Button>(0, 245, 130, 150, 30, m_graphicsManager,
       observer, "Save", 1);
   auto button2 = make_shared<Button>(0, 400, 130, 150, 30, m_graphicsManager,
       observer, "Cancel", 1);

   m_inputSettingsWidgetGroup = make_shared<WidgetGroup>(0, 100, 100, 600,
       250, m_graphicsManager);
...

 {
   ...
   auto label1 = make_shared<Label>(325, 390, 150, 30, " - Graphics - ",
       m_graphicsManager);
   auto label2 = make_shared<Label>(30 + 100, 310, 80, 20, "FOV:",
       m_graphicsManager, 1);

   m_fovSlider = make_shared<Slider>(2, 125 + 100, 310, 150, 30,
       m_graphicsManager, observer, 1);
   m_fovTextInput = make_shared<TextInput>(666, 285 + 100, 310, 100, 30,
       m_graphicsManager, 1);
   ...
   auto button1 = make_shared<Button>(0, 245, 130, 150, 30, m_graphicsManager,
       observer, "Save", 1);
   auto button2 = make_shared<Button>(1, 400, 130, 150, 30, m_graphicsManager,
       observer, "Cancel", 1);
...

 {
   ...
   m_masterVolumeInput = make_shared<TextInput>(666, 485, 310, 50, 30,
       m_graphicsManager, 1);

   auto label3 = make_shared<Label>(230, 270, 80, 20, "Music Volume:",
       m_graphicsManager, 1);
   ...
   auto label4 = make_shared<Label>(230, 230, 80, 20, "Sound Volume:",
       m_graphicsManager, 1);
   m_soundVolumeSlider = make_shared<Slider>(2, 325, 230, 150, 30,
       m_graphicsManager, observer, 1);
   m_soundVolumeSlider->setValue(m_soundPlayer.getSoundVolume());
   m_soundVolumeInput = make_shared<TextInput>(666, 485, 230, 50, 30,
       m_graphicsManager, 1);
