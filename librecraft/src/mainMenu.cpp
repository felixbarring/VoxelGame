...
MainMenu::MainMenu(Game *game)
		...
		case 15: {
			...
			if (name.size()) {
				if (!world_meta::worldNameExists(name)) {
					...
					m_activeWidgetGroup = m_errorPopup;
				}
			} else {
				m_activeWidgetGroup = m_errorPopup;
			}
	...
	shared_ptr<IWidget> button33(new Button{11, 245, 130, 150, 30, observer, "Save", 1});
	shared_ptr<IWidget> button34(new Button{11, 400, 130, 150, 30, observer, "Cancel", 1});
	...
	shared_ptr<IWidget> label8(new Label{30 + 100, 270, 80, 20, "Render Distance:", 1});
	m_slider2.reset(new Slider{101, 125 + 100, 270, 150, 30, observer, 1});
	m_textInput2.reset(new TextInput{666, 285 + 100, 270, 100, 30, 1});

	shared_ptr<IWidget> button13(new Button{12, 245, 130, 150, 30, observer, "Save", 1});
	shared_ptr<IWidget> button14(new Button{13, 400, 130, 150, 30, observer, "Cancel", 1});
	...
	shared_ptr<IWidget> label16(new Label{250, 330, 150, 30, "Enter a Name:", 1});
	m_textInput3.reset(new TextInput{666, 250, 290, 300, 30, 1});
	shared_ptr<IWidget> button15(new Button{14, 240, 135, 100, 30, observer, "Advanced", 1});
	shared_ptr<IWidget> button16(new Button{15, 350, 135, 100, 30, observer, "Create", 1});
	shared_ptr<IWidget> button17(new Button{16, 460, 135, 100, 30, observer, "Cancel", 1});
	shared_ptr<IWidget> button18(new Button{17, 250, 250, 300, 30, observer, "Random Name", 1});
	...
	shared_ptr<IWidget> button19(new Button{18, 225, 60, 80, 30, observer, "Rename", 1});
	shared_ptr<IWidget> button20(new Button{19, 315, 60, 80, 30, observer, "Delete", 1});

	shared_ptr<IWidget> button21(new Button{20, 405, 60, 80, 30, observer, "Load", 1});
	shared_ptr<IWidget> button22(new Button{21, 495, 60, 80, 30, observer, "Cancel", 1});
	...
	shared_ptr<IWidget> label18(new Label{210, 260, 150, 25, "Error: Name can not be empty", 4});
	shared_ptr<IWidget> button23(new Button{22, 510, 260, 80, 30, observer, "OK", 4});
	m_errorPopup.reset(new WidgetGroup{0, 200, 250, 400, 45, observer, 3});
	m_errorPopup->addWidget(label18);
	m_errorPopup->addWidget(button23);
