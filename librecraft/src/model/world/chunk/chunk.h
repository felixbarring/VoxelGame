...
public:
   ...
   * destroyed by the graphics::ChunkBatcher.
   */
  ~Chunk();
   ...
   * Chunk. When loading a group of Chunks, first call Chunk::create. Then
   * Chunk::doSunLightning followed by collecting the light from the neighbors
   * that are old/not in the new group of created/loaded Chunks. Then call
   * Chunk::pChunkropagateLights to complete the lightning work that is prepared
   * by doSunLightning and collecLightFrom* functions. The reason why these are
   * not a single function is to make it possible to run the code in parallel.
   */
  ///@{

  /**
   * This function fills the Chunk with data for the cubes. If there exists a
   * file that corresponds to the location and worldName, the data will be
   * loaded from file. Otherwise it will be generated using noise algorithms.
   * This function can be run in parallel with other instances of Chunk, it will
   * not access data from neighbor Chunks or any other data that is not read
   * only.
   *
  ...
  /**
   * This function spreads light from the sky downwards. The result is stored in
   * the Chunk and will be used by Chunk::propagateLights to do the lightning
   * computations. This function can be run in parallel with other instances of
   * Chunk, it will not access data from neighbor Chunks or any other data that
   * is not read only.
   */
  void doSunLightning();
  ...
  /**
   * This function checks if the light in the right neighbor should light up
   * cubes in this Chunk, and if so adds light to the cubes that will be used in
   * Chunk::propagateLights. It is not safe to run this in parallel with a
   * neighbor. It is however safe to run in parallel with other Chunk instances
   * that are not a neighbor.
   */
   ...
   * It is the callers responsibility to make sure that the arguments are in the
   * correct range. The range should be between 0 and the chunk width, height,
   * depth specified in data::chunk_data.
   *
   ...
   * It is the callers responsibility to make sure that the arguments are in the
   * correct range. The range should be between 0 and the chunk width, height,
   * depth specified in data::chunk_data. If the Voxel id is
   * data::cube_data::BED_ROCK the id will not be changed. Else the graphics
   * will be updated and the Chunk will be marked as dirty, meaning that it
   * needs to be saved to disk when destroyed.
   *
   * @param x The x location of the Voxel
   * @param y The y location of the Voxel
   * @param z The z location of the Voxel
   ...
   * this chunk to its neighbors and also all the pointers in the neighbors
   * pointing to this Chunk
   */
...
private:
  ...
  const unsigned char m_directSunlight = config::graphics_data::directSunlightValue;
