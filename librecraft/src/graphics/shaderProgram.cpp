...
void ShaderProgram::setUniform3f(std::string uniformName, float x, float y,
		float z) {
	glUniform3f(uniforms.find(uniformName)->second, x, y, z);
}

void ShaderProgram::setUniformMatrix3f(std::string uniformName,
		glm::mat3& matrix) {
	glUniformMatrix3fv(uniforms.find(uniformName)->second, 1, GL_FALSE,
			&matrix[0][0]);
}

void ShaderProgram::setUniformMatrix4f(std::string uniformName,
		glm::mat4& matrix) {
