...
public:
  ...
  bool isCubeInFrustum(float x,
    ...
    for (int p = 0; p < 6; p++) {
      ...
      if (glm::dot(plane3, glm::vec3(x - xSize, y - ySize, z - zSize)) +
            plane[3] >
          0)
        continue;
      if (glm::dot(plane3, glm::vec3(x + xSize, y - ySize, z - zSize)) +
    ...
    /*
    for(int p = 0; p < 6; p++ ) {
        if( m_planes[p][0] * (x - xSize) + m_planes[p][1] * (y - ySize) +
    m_planes[p][2] * (z - zSize) + m_planes[p][3] > 0 )
         continue;
        if( m_planes[p][0] * (x + xSize) + m_planes[p][1] * (y - ySize) +
    m_planes[p][2] * (z - zSize) + m_planes[p][3] > 0 )
         continue;
