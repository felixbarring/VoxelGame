...
void ViewDirection::setViewDirection(float horizontal, float vertical) {
  ...
  m_direction = glm::vec3{cos(m_verticalAngle) * sin(m_horizontalAngle), sin(
      m_verticalAngle), cos(m_verticalAngle) * cos(m_horizontalAngle)};
...
void ViewDirection::setSensitivity(double x, double y) {
  m_sensX = x;
  m_sensY = y;
}
