    ...
    const std::vector<short> &elementData)
{
  ...
  for (auto vbo : vbos) {
    ...
    glBufferData(GL_ARRAY_BUFFER, data.size() * FLOAT_SIZE, &data[0], GL_STATIC_DRAW);
    ...
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, elementData.size() * SHORT_SIZE, &elementData[0], GL_STATIC_DRAW);
  }
